---
name: tailor-resume
description: Tailor resume.tex for a specific job—fit report, honest tailored resume, change summary, PR-ready outputs.
---
...
- Read `resume.tex` before editing anything.
- JD requirements not supported by `resume.tex` go in fit report **Do not add** only—never into `tailored-resume.tex`.
...
3. **Tailor** — Load `references/keyword-alignment.md` and `references/latex-resume-structure.md`. Write `tailored-resume.tex` by rephrasing/reordering/emphasizing facts from `resume.tex` only.
...
Commit all three files on the agent branch. Do not modify `resume.tex` on `main`.
