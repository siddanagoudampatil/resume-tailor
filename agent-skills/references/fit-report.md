...
| **Stretch** | `**Verdict:** Too far a stretch for this role - try closer-aligned roles` | Prefer roles aligned closer to the skills already supported by `resume.tex` instead of forcing this application. |
...
Bullets tying JD requirements to evidence in `resume.tex`.
...
List JD items with **no** support in `resume.tex`. These must not appear in `tailored-resume.tex`.
